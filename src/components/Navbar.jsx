                                ...
                                <div className="hidden md:flex items-center space-x-4">
                                    {status === "authenticated" ? (
                                        <>
                                            ...
                                            <span className="font-semibold text-blue-600">{session.user.name || session.user.email}</span>
                                            <button onClick={() => signOut({ callbackUrl: "/login" })} className="bg-blue-500 text-white px-3 py-2 rounded-lg font-semibold hover:bg-blue-600 transition">Logout</button>
                                        </>
                                    ) : (
                                ...
                                <div className="flex flex-col items-center py-4 gap-4 font-semibold">
                                        ...
                                        {status === "authenticated" ? (
                                            <div className="flex flex-col items-center gap-2 mt-2">
                                                ...
                                                <span className="font-semibold text-blue-600">{session.user.name || session.user.email}</span>
                                                <button onClick={() => { setMenuOpen(false); signOut({ callbackUrl: "/login" }); }} className="bg-blue-500 text-white px-3 py-2 rounded-lg font-semibold hover:bg-blue-600 transition">Logout</button>
                                            </div>
                                        ) : (
                                            <div className="flex space-x-4 text-white font-semibold bg-blue-500 py-2 px-4 rounded-lg">
